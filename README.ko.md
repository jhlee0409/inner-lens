...
│                        애플리케이션 (브라우저)                            │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────┐ │
│  │ 사용자가    │───►│ 위젯이       │───►│ 클라이언트 측 마스킹         │ │
│  │ 버그 제보   │    │ 컨텍스트     │    │ (30개 패턴 적용)            │ │
│  │ 클릭       │    │ 캡처         │    │ • 이메일 → [EMAIL_REDACTED] │ │
│  └─────────────┘    └──────────────┘    │ • API 키 → [*_REDACTED]    │ │
│                                         │ • 토큰 → [TOKEN_REDACTED]  │ │
│                                         └──────────────┬──────────────┘ │
└────────────────────────────────────────────────────────┼────────────────┘
...
| 데이터 유형 | 목적 | 개인정보 보호 |
|------------|------|--------------|
| 콘솔 로그 | 에러 & 경고 디버깅 | 민감 정보 자동 마스킹 |
| 사용자 액션 | 사용자 여정 파악 | 일반 셀렉터만 (개인 텍스트 제외) |
| 네비게이션 히스토리 | 페이지 흐름 추적 | URL만 (쿼리 파라미터 마스킹) |
| 브라우저 & OS 정보 | 환경 디버깅 | 요약만 (예: "Chrome 120", "Windows 10") |
| 성능 메트릭 | 느린 작업 식별 | 타이밍 데이터만 (LCP, FCP 등) |
| DOM 스냅샷 (선택) | 시각적 디버깅 | 기본값 비활성화 |

### 수집하지 않는 데이터

- ❌ **IP 주소** — 로깅 또는 저장 안 함
- ❌ **쿠키** — 접근 안 함
- ❌ **localStorage/sessionStorage** — 읽지 않음
- ❌ **위치 정보** — 요청 안 함
- ❌ **디바이스 핑거프린트** — 고유 식별자 없음
- ❌ **폼 필드 값** — 입력 내용 제외
- ❌ **전체 User Agent 문자열** — 브라우저/OS 요약만

### 자동 마스킹 (30개+ 패턴)

민감한 데이터는 **전송 전 클라이언트 측에서** 마스킹됩니다:
...
1. **클라이언트 우선** — 모든 마스킹이 브라우저에서 발생 (서버 전송 전)
2. **데이터 미보관** — Hosted API는 패스스루; 로그 저장 없음
3. **사용자 주도** — 사용자가 제출 클릭 시에만 리포트 전송
4. **최소 수집** — 디버깅에 필요한 데이터만 캡처
5. **투명한 저장** — 데이터는 당신의 GitHub 저장소에만 저장
