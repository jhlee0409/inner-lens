...
## ⛔ 제안 → 구현 플로우 (BLOCKING - 순서 어기면 금지)

### 수정/개선 제안이 나오면 반드시 이 순서로:
...
**2단계: 타당성 검증 (구현 전)**
```
- 제안한 방식이 기존 타입과 호환되는가?
- 제안한 방식이 기존 호출 체인에 통합 가능한가?
- 숨겨진 의존성이나 부작용은 없는가?
- 더 간단한 방법은 없는가?
```
→ 검증 결과를 사용자에게 보여주고 승인받기
...
**5단계: 검증**
```bash
# 새로 만든/수정한 함수가 실제로 호출되는지 증명
grep -rn "함수명(" --include="*.ts" | grep -v test | grep -v "\.d\.ts"
```
- 호출 경로를 진입점부터 나열
- 기존 로직이 여전히 작동하는지 확인

**제안만 하고 바로 구현하지 말 것. 반드시 2단계 타당성 검증 먼저.**

---

## ⛔ 구현 전 필수 (BLOCKING - 이거 안 하면 구현 금지)

### 모든 구현/수정 요청 시 반드시 먼저:

**1. 영향 분석 (구현 전에 사용자에게 보여주기)**
```
[영향 분석]
- 수정할 파일: 
- 수정할 함수:
- 이 함수를 호출하는 곳: (grep으로 확인)
- 수정할 파일에 정의된 함수 중 호출 안 되는 것: (grep으로 확인)
- 이 변경으로 제거/우회되는 기존 로직:
- 제거되는 로직이 하던 일:
- 새 구현에서 그 기능이 대체되는가: Y/N
- 대체 안 되면 의도적 제거인가 실수인가:
```

**2. 승인 대기**
- 위 분석을 사용자에게 보여주고 승인받기 전까지 코드 수정 금지

**3. 구현 후 증명 (완료 선언 전 필수)**
```bash
# 새로 만든/수정한 함수가 실제로 호출되는지 증명
grep -rn "함수명(" --include="*.ts" | grep -v test | grep -v "\.d\.ts"
```
- 호출 횟수 0이면 "완료" 아님
- 호출 경로를 진입점부터 나열해서 보여주기

**4. 완료 조건 (모두 충족해야 함)**
- [ ] 영향 분석 완료 및 사용자 승인
- [ ] typecheck 통과
- [ ] test 통과
- [ ] 새 함수가 호출되는 경로 grep으로 증명
- [ ] 기존에 있던 검증/연결 로직이 여전히 작동하는지 확인

**"테스트 통과했습니다"만으로 완료 선언 금지.**
